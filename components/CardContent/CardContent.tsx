...
const CardContent: FC<CardContentProps> = ({ collection, variant }) => {
  ...
  return (
    <div className={`w-full lg:w-10/12 mt-16 ${variant === 'left' ? 'ml-0' : 'ml-0 lg:ml-[16.67%]'}`}>
      <div
      ...
      <div className={`${classes['items-container']} ${variant === 'right' ? classes['items-container-right'] : ''}`}>
        <div className="p-3">
        ...
        <div className={classes['logo-container']}>
          {items.map(({ element, alt, width, heigth }, i) => (
            <Image key={`${alt}-${i}`} src={element} alt={alt} width={width} height={heigth} priority />
          ))}
        </div>
      </div>
    </div>
